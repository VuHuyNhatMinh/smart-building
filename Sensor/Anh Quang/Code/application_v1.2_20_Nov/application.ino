 ...
 * Nhiệt độ ,độ ẩm, Ánh sáng, màu sắc , CO2 MH Z19, hoạt động tốt
...
uint16_t eCO2 = 0;          /*eCO2 CCS811 Global Variable*/
uint16_t TVOC = 0;          /*TVOC CCS811 Global Variable*/
...
void setup() {
  ...
  timer3.setInterval(500);    /* Sample time of TSL2561 Lux Sensor */
  timer4.setInterval(500);    /* Sample time of RGB Color Sensor */
  timer5.setInterval(5000);   /* Sample time of CO2 Mh Z19 Sensor */
  timer6.setInterval(5000);   /* Sample time of GP2y1010AuF Dust Sensor */
  timer7.setInterval(5000);   /* Sample time of eCO2 and TVOC CSS811 Sensor */
  timer8.setInterval(15000);  /* Sample time of Sound MAX9814 Sensor */
  timer9.setInterval(20000);  /* Sample timer of Data Transmission UART */
...
{
  g_dustDensity = dustSensor.getDustDensity();
}
/*
...
void DateTimeDisplay(void)
{
  ...
  display.print(daysOfTheWeek[theDayOfWeek]);
...
void DisplayAllIndex(void)
{
  ...
  display.setCursor(0, 13);          //Setting the cursor position
  display.print("T:");              //Display the temperature and humidity as "T: 24.64 C
  display.print(g_Temper);
  // display.drawCircle(44, 13, 1, WHITE);
  display.print(" C");
  display.setCursor(64, 13);
  display.print("H:");
  display.print(g_Humid);
  display.print(" %");

  display.setTextSize(1);            //Setting the text size and color
  display.setTextColor(WHITE);

  display.setCursor(0, 28);          //Setting the cursor position
  display.print("Lux:");              //Display the temperature and humidity as "T: 24.64 C
  display.print(g_Lux);

  display.setTextSize(1);            //Setting the text size and color
  display.setTextColor(WHITE);

  display.setCursor(64, 28);          //Setting the cursor position
  display.print("Noise:");              //Display the temperature and humidity as "T: 24.64 C
  display.print(g_noiseEvent);

  display.setTextSize(1);            //Setting the text size and color
  display.setTextColor(WHITE);

  display.setCursor(0, 57);          //Setting the cursor position
  display.print("R:");              //Display the temperature and humidity as "T: 24.64 C
  display.print(g_Red);               //                                         H: 59.12 %"
  display.setCursor(42, 57);
  display.print("G:");
  display.print(g_Green);
  display.setCursor(84, 57);
  display.print("B:");
  display.print(g_Blue);


  display.setTextSize(1);            //Setting the text size and color
  display.setTextColor(WHITE);

  display.setCursor(0, 43);          //Setting the cursor position
  display.print("CO2:");
  display.print(g_CO2);               //
  //display.print("ppm");
  ...
  display.setCursor(64, 43);          //Setting the cursor position
  display.print("DUST:");              //Display the temperature and humidity as "T: 24.64 C
  display.print(g_dustDensity);
  //display.print(" mg/m3");
...
void DataTransmission(void)
{
  ...
  root["sensorid1"] = 1;
  root["time"] = theUnixtime;
  root["temperature"] = g_Temper;
  root["humidity"] = g_Humid;
  root["light"] = g_Lux;
  root["g_Red"] = g_Red;
  root["g_Green"] = g_Green;
  root["g_Blue"] = g_Blue;
  root["co2"] = g_CO2;
  root["sound"] = g_noiseEvent;
  root["dust"] = g_dustDensity;
  root["people"] = "1";
