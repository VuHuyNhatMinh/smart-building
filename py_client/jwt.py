...
class JWTClient:
    ...
    base_endpoint = "http://localhost:8000/api/v1/sensors"
    ...
    def list(self, endpoint=None, limit=3):
        ...
        if endpoint is None or self.base_endpoint not in str(endpoint):
            ...
            endpoint = f"{self.base_endpoint}/listCreate1/"  
    ...
    def list(self, endpoint=None):
        ...
        if endpoint is None or self.base_endpoint not in str(endpoint):
            # endpoint = f"{self.base_endpoint}/products/?limit={limit}"
            endpoint = f"{self.base_endpoint}/mixin/get/"  
        r = requests.get(endpoint, headers=headers) 
        if r.status_code != 200:
            raise Exception(f"Request not complete {r.text}")
        data = r.json()
    ...
    def post(self, endpoint=None, data=None):
        ...
        if endpoint is None or self.base_endpoint not in str(endpoint):
            # endpoint = f"{self.base_endpoint}/products/?limit={limit}"
            endpoint = f"{self.base_endpoint}/mixin/post/"  
        r = requests.post(endpoint, headers=headers, json=data) 
        if r.status_code != 201:
            raise Exception(f"Request not complete {r.text}")
        data = r.json()
        return data
        #{'id': 2710, 'mac': '4C:75:25:06:A1:E7', 'temp': 111, 'hum': 222}
...
if __name__ == "__main__":
   ...
   """
    data = {'mac': '4C:75:25:06:A1:E7', 'temp': 111, 'hum': 222}
    client = JWTClient() 
    print(client.post(data=data))